...
# From: <https://github.com/BigWigsMods/packager/blob/master/release.sh>
	...
	( cd "$_cdt_srcdir" && eval "$_cdt_find_cmd" ) | while read -r file; do
		...
		if [ -f "$_cdt_srcdir/$file" ]; then
			...
			if [ -n "$skip_copy" ]; then
				...
				# SwimmingTiger: replacing db/*.lua is too slow, so only replace the toc file.
				#file_pattern="*.lua:*.md:*.toc:*.txt:*.xml"
				file_pattern="*.toc"
				if match_pattern "$file" "$file_pattern"; then
					skip_filter=
				fi
